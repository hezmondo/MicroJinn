...
emailacc_bp = Blueprint('emailacc_bp', __name__)


@emailacc_bp.route('/email_accounts', methods=['GET'])
def email_accounts():
...
@emailacc_bp.route('/email_account/<int:id>', methods=['GET', 'POST'])
@login_required
def email_account(id):
    if request.method == "POST":
        id = post_emailaccount(id)
        return redirect(url_for('emailacc_bp.email_account', id=id))
