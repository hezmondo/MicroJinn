...
from app.dao.functions import dateToStr, hashCode, money, moneyToStr, round_decimals_down
from app.dao.rent import get_last_date_from_datecode


def get_rent_gale(datecode, frequency, rent_date, rentpa):
    if rentpa == 0:
        return money(rentpa)
    missing_pennies = (rentpa * 100) % frequency
    if missing_pennies == 0:
        return money(rentpa / frequency)
    last_date_entry = get_last_date_from_datecode(datecode)
    # Some rents have date codes that are not in the dates table, in which case we cannot round accurately
    if last_date_entry:
        rent_gale = money(round_decimals_down(rentpa / frequency))
        if rent_date.month == last_date_entry.month and rent_date.day == last_date_entry.day:
