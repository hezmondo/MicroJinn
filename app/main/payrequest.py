...
from app.dao.charge import add_charge, get_charge_start_date, get_charge_type, get_total_charges, get_charges_rent
...
def build_pr_charges_table(rent_pr):
    charges = get_charges_rent(rent_pr.id)
    charge_table_items = {}
    new_charge_dict = {}
    total_charges = 0
...
def build_pr_table(rent_pr, pr_variables):
    ...
    nextrentdate = inc_date_m(rent_pr.lastrentdate, rent_pr.freq_id, rent_pr.datecode_id, 1) if hasattr(rent_pr,
                                'lastrentdate') else datetime.date(1991, 1, 1)
    rent_gale = get_rent_gale(nextrentdate, rent_pr.freq_id, rent_pr.rentpa)
    ...
    table_rows = {}
    if rent_gale:
        rent_statement = build_rent_statement(rent_pr, rent_type, nextrentdatestr)
...
def build_rent_statement(rent_pr, rent_type, nextrentdatestr):
